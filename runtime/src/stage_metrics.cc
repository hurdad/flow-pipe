...
static opentelemetry::nostd::shared_ptr<opentelemetry::metrics::Meter>
GetMeter() {
...
// Queue metrics
// ------------------------------------------------------------
void StageMetrics::RecordQueueDequeue(
  ...
  const uint64_t dwell_ns =
      payload.meta.enqueue_ts_ns > 0
          ? now_ns - payload.meta.enqueue_ts_ns
          : 0;
...
void StageMetrics::RecordQueueEnqueue(
    const QueueRuntime& queue) noexcept {
...
// Stage metrics
// ------------------------------------------------------------
void StageMetrics::RecordStageLatency(
...
void StageMetrics::RecordStageError(
