...
Use [`e2e-k3s.sh`](./e2e-k3s.sh) to provision a local k3s-based Kubernetes cluster, build and load Flow Pipe images, install the Helm chart (observability disabled by default), and exercise flows end-to-end via the API using the `flow-pipe-cli` container image.
...
- `OBSERVABILITY_ENABLED`: Enables observability exports when set to `true` (default: `false`)
...
3. Installs the Helm chart from `deploy/helm/flow-pipe` with observability disabled by default (set `OBSERVABILITY_ENABLED=true` to enable exports), then waits for the control plane pods.
